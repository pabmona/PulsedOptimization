...
IntAncho    = [2,11];             % Intervalo de valores de ancho de pulso
...
IntGanancia = [1,6];
NumPuntPuls = 8;                  % Número de puntos de pulso
sigma_adm   = 60e6;               % Tensión máxima admisible
FormaPolino = 9;				  % n -> n-1 capas ---> n = 9 -> 8 capas
Th          = 50+273.15;
e           = 1e-3;
...
Kmax = 100*(1+NumPuntPuls+FormaPolino+1);   % Unas 100 veces el numero de parametros
Kaccept = 10*(1+NumPuntPuls+FormaPolino+1); % Unas 10 veces el numero de parametros
...
GananciaIP = 2*ones(1,NumPuntPuls);       % Pulso cuadrado, de ganancia 2,5.
...
HisParam = zeros(1+NumPuntPuls+FormaPolino+1+7,Almacen);% Variable que acumula los pulsos considerados y la determinación tomada.
...
iter     = 0;                               % La iteración
PARAM    = [AnchoPul,GananciaIP,PuntosGeo,Longitud]; % Parámetros iteracion previa
PARAMN   = PARAM;                           % Parámetros nueva Iteracion
m        = length(PARAM);                   % 
...
while ~acabar
    ...
    crearicorr(Th,PARAMN(1+NumPuntPuls+(1:FormaPolino)),PARAMN(m));
    inputsfeap(NombreFichero,PARAMN(m));
    crearipulse(PARAMN(1+(1:NumPuntPuls)),PARAMN(1),dtc);
    ...
    % HALLAR M�?NIMO DE TEMPERATURA. CON PRECISIÓN.
    ...
    if ~acabar
        ...
        elseif j >= 2 && j <= 1+NumPuntPuls
            PARAMN(j) = IntGanancia(1)+rand(1)^3*(IntGanancia(2)-IntGanancia(1));
        else
    ...
    if ~mod(iter,5)
        save('resultados','PARAM','PARAMN','iteracep','iterrech','numKmax','x2','HisParam','O1','f1','f2','f3','f4','t','iter','tens','tiem','temp')
    end
